...
-- ==========================================================
-- SCRIPT DE BASE DE DATOS - SPRINT 2.6
...
-- 5. AUTOMATIZAR LA CREACIÓN DE MOVIMIENTO DE ENTRADA AL CREAR UN LOTE (ATRIBUCIÓN SEGURA)
...
BEGIN
    ...
    -- Exigir estrictamente que el usuario esté autenticado para registrar el lote
    IF v_user_id IS NULL THEN
        RAISE EXCEPTION 'No se puede registrar el lote sin un usuario autenticado.';
    END IF;
...
-- NOTA DE SEGURIDAD: El permiso de INSERT directo concedido al rol authenticated 
-- es una solución temporal y de transición para este Sprint. Se planea reemplazarlo
-- por funciones RPC controladas con SECURITY DEFINER cuando se implemente el módulo 
-- definitivo de ventas y ajustes, bloqueando por completo la inserción directa.
